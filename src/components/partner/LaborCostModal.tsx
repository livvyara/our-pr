...
const LaborCostModal: React.FC<Props> = ({ 
  ...
  const [taxConfig, setTaxConfig] = useState<TaxConfig>({ 
      freelance: 3.3, 
      agency: { dailyDeduction: 150000, incomeTaxRate: 2.7, localTaxRate: 10, eiRate: 0.9 } 
  });
  ...
  useEffect(() => {
      ...
      const loadBasics = async () => {
          ...
          if (taxSnap.exists()) {
              ...
              if (typeof tData.agency === 'number') {
                 // 구버전 데이터 처리 (필요시)
              } else {
                  setTaxConfig(tData as TaxConfig);
              }
          }
  ...
  const calculate = () => {
      ...
      if (type === 'freelance') {
          ...
          let incomeTax = 0;
          if (taxBase > 0) {
              incomeTax = Math.floor(taxBase * ((taxConfig.agency.incomeTaxRate || 2.7) / 100));
          }
  ...
  const handleSave = async () => {
      ...
      try {
          ...
          const data: any = {
              ...
              workerType: worker.workerType || 'freelance',
              companyName: worker.companyName, 
              siteId: selectedSiteId, 
              siteName: site ? site.siteName : defaultSiteName || '알수없음',
              paymentMonth, 
              workedDays: Array.from(selectedDays).sort((a, b) => a - b), 
              totalDays: selectedDays.size,
              preTaxAmount, 
              deductionAmount: calcResult.totalDeduction, 
              finalAmount: calcResult.final,
              paymentCycle, 
              isTaxExempt, 
          ...
          if (targetLabor?.id) {
              ...
              if (userName) {
                  await addDoc(collection(db, 'users', partnerUid, 'activityLogs'), {
                      text: `[노무수정] ${userName}님이 ${worker.workerName}님의 ${paymentMonth} 노무 내역을 수정했습니다.`,
                      createdAt: serverTimestamp(), type: 'hr_labor_update'
                  });
              }
          } else {
            ...
            <div className="lm-row">
                ...
                <div className="lm-group half">
                    ...
                    <select 
                        ...
                        style={defaultSiteId ? {backgroundColor: '#e9ecef'} : {}}
                    >
