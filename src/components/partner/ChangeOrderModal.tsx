...
      switch(status) {
          case 'pending_partner': return <span className="change-order-modal-badge pending">대표승인 대기</span>;
          case 'pending_customer': return <span className="change-order-modal-badge customer">고객승인 대기</span>;
          case 'approved': return <span className="change-order-modal-badge approved">최종승인 완료</span>;
          default: return <span>-</span>;
      }
  };

  return (
    <div className="change-order-modal-overlay">
      <div className="change-order-modal-content">
        <div className="change-order-modal-header">
          <h3>추가/변경 견적 협의</h3>
          <button className="change-order-modal-close-btn" onClick={onClose}>×</button>
        </div>

        <div className="change-order-modal-body">
            {activeTab === 'list' && (
                <>
                    <div style={{marginBottom:'15px', display:'flex', justifyContent:'flex-end'}}>
                        {userRole !== 'customer' && (
                            <button className="change-order-modal-btn-new" onClick={() => setActiveTab('form')}>+ 신규 등록</button>
                        )}
                    </div>
                    <div className="change-order-modal-list">
                        {orders.length === 0 ? <p className="change-order-modal-list no-data">등록된 내역이 없습니다.</p> : 
                        orders.map(order => (
                            <div key={order.id} className="change-order-modal-item" onClick={() => { setSelectedOrder(order); setActiveTab('detail'); }}>
                                <div className="change-order-modal-item-header">
                                    <span className="change-order-modal-date">{order.createdAt?.toDate().toLocaleDateString()}</span>
                                    {getStatusBadge(order.status)}
                                </div>
                                <div className="change-order-modal-item-title">{order.title}</div>
                                <div className="change-order-modal-item-amount">합계: {order.totalAmount.toLocaleString()} 원</div>
                                <div className="change-order-modal-item-author">작성자: {order.authorName}</div>
                            </div>
                        ))
                        }
                    </div>
...
            {activeTab === 'form' && (
                <div className="change-order-modal-form">
                    <div className="change-order-modal-form-group">
                        <label>제목 (변경 사유)</label>
                        <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="예: 거실 조명 추가 요청건" />
                        
                    </div>
                    
                    <div className="change-order-modal-table-wrapper">
                        <table className="change-order-modal-input-table">
                            <thead>
                                <tr>
...
                                        <td><button onClick={() => removeItemRow(idx)} className="change-order-modal-btn-del-row">×</button></td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                    </div>
                    <button className="change-order-modal-btn-add-row" onClick={addItemRow}>+ 행 추가</button>

                    <div className="change-order-modal-total-box">
                        총 합계: <strong>{getTotalAmount().toLocaleString()} 원</strong>
                    </div>

                    <div className="change-order-modal-actions">
                        <button className="change-order-modal-btn-cancel" onClick={() => setActiveTab('list')}>취소</button>
                        <button className="change-order-modal-btn-save" onClick={handleSubmit} disabled={isSubmitting}>등록하기</button>
                    </div>
                </div>
            )}

           {activeTab === 'detail' && selectedOrder && (
                <div className="change-order-modal-detail">
                    <div className="change-order-modal-detail-header">
                        <h4>{selectedOrder.title}</h4>
                        {getStatusBadge(selectedOrder.status)}
                    </div>
                    <p className="change-order-modal-detail-meta">작성일: {selectedOrder.createdAt?.toDate().toLocaleDateString()} | 작성자: {selectedOrder.authorName}</p>

                    <div className="change-order-modal-table-wrapper">
                        <table className="change-order-modal-view-table">
                            <thead>
...
                                    <tr key={i}>
                                        <td className="text-left" data-label="품명">{item.name}</td>
                                        <td className="text-center" data-label="단위">{item.unit}</td>
                                        <td className="text-center" data-label="수량">{item.quantity}</td>
                                        <td className="text-right" data-label="단가">{item.unitPrice}</td>
                                        <td className="text-right" data-label="공급가액">{item.supplyPrice.toLocaleString()}</td>
                                        <td className="text-right" data-label="세액">{item.tax.toLocaleString()}</td>
                                        <td className="text-right font-bold" data-label="합계">{item.totalPrice.toLocaleString()}</td>
...
                    <div className="change-order-modal-actions">
                        <button className="change-order-modal-btn-back" onClick={() => setActiveTab('list')}>목록으로</button>
                        {userRole === 'partner' && selectedOrder.status === 'pending_partner' && (
                            <button className="change-order-modal-btn-approve" onClick={() => handleApprove(selectedOrder)}>승인 (고객에게 전송)</button>
                        )}
                        {userRole === 'customer' && selectedOrder.status === 'pending_customer' && (
                            <button className="change-order-modal-btn-approve" onClick={() => handleApprove(selectedOrder)}>최종 승인</button>
                        )}
                    </div>
                </div>
            )}
