...
const ConstructionScheduleModal: React.FC<ModalProps> = ({ siteId, partnerUid, onClose }) => {
  ...
  const handleDelete = async () => {
    ...
    try { 
        await deleteDoc(doc(db, 'users', partnerUid, 'sites', siteId, 'schedules', editId)); 
        handleCancelEdit(); 
        alert("삭제되었습니다.");
                            ...
                            {week.map((day, dIdx) => (
                                <td key={dIdx}>
                                    ...
                                    {day.schedules.map((sched: any) => (
                                        <div key={sched.id} onClick={(e) => { e.stopPropagation(); handleEdit(sched); }} style={{cursor:'pointer'}}>
