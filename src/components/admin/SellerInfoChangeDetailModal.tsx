// src/components/admin/SellerInfoChangeDetailModal.tsx

import React, { useState, useMemo } from 'react';
import { httpsCallable } from 'firebase/functions';
import './SellerInfoChangeDetailModal.css'; // [수정]
import { Timestamp } from 'firebase/firestore'; 
import { auth, functions } from '../../firebase-config'; 

// (셀러 정보 인터페이스 - 파트너와 동일하다고 가정)
interface SellerInfo {
  ...
  file2Url: string | null; // (예: 통신판매업신고증)
}
interface ChangeRequestData {
  uid: string; // 문서 ID
  userId: string; // 사용자 UID
  currentInfo: SellerInfo; 
  requestedInfo: SellerInfo; 
  status: 'pending' | 'approved' | 'rejected';
  newLicenseUrl?: string | null; // (신규 통신판매업)
  newAttachmentUrls?: string[]; // (신규 사업자등록증 등)
  createdAt: Timestamp;
}
...
const SellerInfoChangeDetailModal: React.FC<ModalProps> = ({ requestData, onClose }) => {
  const [isLoading, setIsLoading] = useState(false);
  const [rejectionReason, setRejectionReason] = useState('');

  // [파일 할당 state]
  const [assignedFile1Url, setAssignedFile1Url] = useState('keep');
  const [assignedFile2Url, setAssignedFile2Url] = useState('keep'); // (예: 통신판매업)

  // [신규 파일 목록]
  const newFileOptions = useMemo(() => {
    const options = [];
    // (셀러 로직에 맞게 newLicenseUrl 이름이 다를 수 있으나, 파트너와 동일한 스키마로 가정)
  ...
  const handleApprove = async () => {
    ...
    setIsLoading(true);
    try {
      // [수정] 'approveSellerInfoChange' 호출 (다음 단계에서 생성)
      const approveFunction = httpsCallable(functions, 'approveSellerInfoChange');
  ...
  const handleReject = async () => {
    ...
    setIsLoading(true);
    try {
      // [수정] 'rejectSellerInfoChange' 호출 (다음 단계에서 생성)
      const rejectFunction = httpsCallable(functions, 'rejectSellerInfoChange');
      ...
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <table className="diff-table">
          ...
          <tbody>
            ...
            {/* 3. 통신판매업신고증 (file2) 렌더링 */}
            <tr>
              <th>통신판매업신고증<br/>(선택)</th>
              <td>
...
export default SellerInfoChangeDetailModal;