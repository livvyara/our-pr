// src/components/admin/SellerApplicationDetailModal.tsx
...
import './SellerApplicationDetailModal.css'; // [수정] .css 파일 임포트

// (셀러 신청서 데이터 타입 - 파트너와 동일하다고 가정)
...
const SellerApplicationDetailModal: React.FC<ModalProps> = ({ application, onClose }) => {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      // [⭐ 수정] 'sellerApplications' 컬렉션에 저장
      const appDocRef = doc(db, 'sellerApplications', application.uid);
  ...
  const handleApprove = async () => {
    if (!window.confirm("정말로 이 신청을 '승인'하시겠습니까? 해당 회원의 등급이 'seller'로 변경됩니다.")) {
      return;
    }
    setIsLoading(true);
    try {
      // 1. Cloud Function('approveSellerApplication') 호출 (다음 단계에서 생성)
      const approveFunction = httpsCallable(functions, 'approveSellerApplication');
      
      // 2. 'users' 문서에 저장할 sellerInfo 데이터 준비
      ...
      await approveFunction({
        ...
        applicationData: sellerInfoData, // 'users' 문서에 저장할 데이터
      });
  ...
  const handleReject = async () => {
    ...
    setIsLoading(true);
    try {
      // 1. Storage 파일 삭제 (셀러 경로)
      const folderRef = ref(storage, `seller-applications/${application.userId}`);
      const fileList = await listAll(folderRef);
      const deletePromises = fileList.items.map(fileRef => deleteObject(fileRef));
      await Promise.all(deletePromises);
      
      // 2. Cloud Function('rejectSellerApplication') 호출 (다음 단계에서 생성)
      const rejectFunction = httpsCallable(functions, 'rejectSellerApplication');
    ...
    <div className="modal-overlay" onClick={() => onClose(false)}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="application-details">
          ...
          <span className="detail-label">통신판매업신고증 (가정)</span>
          <span className="detail-value">
