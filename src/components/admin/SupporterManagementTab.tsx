// src/components/admin/SupporterManagementTab.tsx

import React, { useState, useEffect, useCallback } from 'react';
import { getFirestore, collection, getDocs, query, where, Timestamp, orderBy } from 'firebase/firestore';
import './SupporterManagementTab.css';
import SupporterApplicationDetailModal from './SupporterApplicationDetailModal';
import SupporterInfoChangeDetailModal from './SupporterInfoChangeDetailModal'; // [⭐ 1. 추가]
...
// Props 정의
interface SupporterManagementTabProps {
  pendingCount: number;
  infoChangeCount: number;
}

const SupporterManagementTab: React.FC<SupporterManagementTabProps> = ({ pendingCount, infoChangeCount }) => {
  // 1. 3개의 메인 탭 상태
  ...
  // '신규 신청' 목록
  const fetchSupporterApplications = useCallback(async (status: string) => {
    setIsLoading(true);
    const appCollectionRef = collection(db, "supporterApplications");
  ...
  // [⭐ 2. 수정] '정보 변경' 목록
  const fetchSupporterChangeRequests = useCallback(async (status: string) => {
    setIsLoading(true);
    const reqCollectionRef = collection(db, "supporterInfoChangeRequests");
  ...
  useEffect(() => {
    if (activeTab === 'newApplications') {
      fetchSupporterApplications(activeNewAppSubTab);
    } else if (activeTab === 'infoChange') {
      fetchSupporterChangeRequests(activeInfoChangeSubTab);
    } else {
      setIsLoading(false);
    }
  }, [activeTab, activeNewAppSubTab, activeInfoChangeSubTab, fetchSupporterApplications, fetchSupporterChangeRequests]);

  // [⭐ 4. 수정] 모달 핸들러 (2종류)
  const handleViewDetails = (app: ApplicationData) => setSelectedApp(app);
  const handleCloseModal = (refresh: boolean) => {
    setSelectedApp(null);
    if (refresh) fetchSupporterApplications(activeNewAppSubTab); 
  };
  ...
  const handleCloseChangeModal = (refresh: boolean) => {
    ...
    if (refresh) fetchSupporterChangeRequests(activeInfoChangeSubTab);
  };


  // 5-A. "서포터 관리" (메인 탭) 렌더링
  const renderMainTab = () => (
    <div className="supporter-tab-content">
      <h3>서포터 관리 (메인)</h3>
      <p>기존 서포터(협력사)들의 정보를 관리하는 서브메뉴입니다. (미구현)</p>
  ...
  // 5-B. "신규 서포터 신청 관리" 탭 렌더링
  const renderNewApplicationsTab = () => (
    <div className="supporter-tab-content">
      
      {/* 6-1. 하위 탭 버튼들 */}
      <div className="supporter-sub-tabs">
        <button
          className={`supporter-sub-tab-button ${activeNewAppSubTab === 'pending' ? 'active' : ''}`}
          onClick={() => setActiveNewAppSubTab('pending')}
        >
          신청 목록
        </button>
        <button
          className={`supporter-sub-tab-button ${activeNewAppSubTab === 'approved' ? 'active' : ''}`}
          onClick={() => setActiveNewAppSubTab('approved')}
        >
          승인 내역
        </button>
        <button
          className={`supporter-sub-tab-button ${activeNewAppSubTab === 'rejected' ? 'active' : ''}`}
      ...
      {/* 6-2. 하위 탭 제목 */}
      <h3>
        {activeNewAppSubTab === 'pending' && '신규 서포터 신청 목록'}
        {activeNewAppSubTab === 'approved' && '서포터 승인 내역'}
        {activeNewAppSubTab === 'rejected' && '서포터 부결 내역'}
      </h3>
      {/* ... (p 태그) ... */}
  ...
  const renderInfoChangeTab = () => (
    <div className="supporter-tab-content">
      ...
      {/* 6-1. 하위 탭 버튼들 (supporter-sub-tabs 클래스 재사용) */}
      <div className="supporter-sub-tabs">
        <button
          className={`supporter-sub-tab-button ${activeInfoChangeSubTab === 'pending' ? 'active' : ''}`}
          onClick={() => setActiveInfoChangeSubTab('pending')}
        >
          신청 목록
        </button>
        <button
          className={`supporter-sub-tab-button ${activeInfoChangeSubTab === 'approved' ? 'active' : ''}`}
          onClick={() => setActiveInfoChangeSubTab('approved')}
        >
          승인 내역
        </button>
        <button
          className={`supporter-sub-tab-button ${activeInfoChangeSubTab === 'rejected' ? 'active' : ''}`}
  ...
  return (
    <div>
      ...
      {/* 1. 내부 메인 탭 (3개) */}
      <div className="supporter-main-tabs">
        <button
          className={`supporter-tab-button ${activeTab === 'main' ? 'active' : ''}`}
          onClick={() => setActiveTab('main')}
        >
          서포터 관리
        </button>
        <button
          className={`supporter-tab-button ${activeTab === 'newApplications' ? 'active' : ''}`}
          onClick={() => setActiveTab('newApplications')}
        >
          <span>신규 서포터 신청 관리</span>
          {pendingCount > 0 && (
            <span className="new-notification">N</span>
          )}
        </button>
        <button
          className={`supporter-tab-button ${activeTab === 'infoChange' ? 'active' : ''}`}
      ...
      {/* 3. 모달 (2종류) */}
      {selectedApp && (
        <SupporterApplicationDetailModal
          application={selectedApp}
          onClose={handleCloseModal}
        />
      )}
      {selectedChangeRequest && (
        <SupporterInfoChangeDetailModal
...
export default SupporterManagementTab;