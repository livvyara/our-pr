...
  return (
    <div className="my-project-page-container">
      {!isMobile && <RoleHeader />}
...
      <main className="my-project-page-main">
        <div className="my-project-page-inner-container">
            <div className="my-project-page-header">
                <h2>마이 프로젝트</h2>
                <p>진행 중인 나의 현장 정보를 한눈에 확인하세요.</p>
            </div>

            {loadingData ? <div className="my-project-page-loading">로딩 중...</div> : (
                <div className="my-project-page-list">
                    {mySites.length === 0 ? (
                    <div className="my-project-page-empty">
                        <p>참여 중인 프로젝트가 없습니다.</p>
                        <span className="my-project-page-sub-text">업체로부터 초대장을 받으면 이곳에 현장이 표시됩니다.</span>
                    </div>
                    ) : (
                    mySites.map(site => {
...
                        return (
                        <div key={site.siteId} className={`my-project-page-card ${isExpanded ? 'expanded' : ''}`}>
                            <div className="my-project-page-card-header" onClick={() => toggleExpand(site.siteId)}>
                                <div className="my-project-page-card-info">
                                    <div className="my-project-page-site-name">
                                        {site.siteName}
                                        <span className="my-project-page-status-badge" style={{color:statusStyle.color, backgroundColor:statusStyle.bg}}>
                                            {statusStyle.text}
                                        </span>
                                    </div>
                                    <div className="my-project-page-site-addr">{site.address}</div>
                                </div>
                                <button className={`my-project-page-btn-toggle ${isExpanded ? 'open' : ''}`}><ChatIcons.Back /></button>
                            </div>

                            <div className="my-project-page-card-body">
                                <div className="my-project-page-detail-grid">
                                    <div className="my-project-page-section">
                                        <h4>📋 공사 개요</h4>
                                        <div className="my-project-page-row"><span>공사 유형</span> {site.siteType === 'commercial' ? '상업공간' : '주거공간'}</div>
                                        <div className="my-project-page-row"><span>면적</span> {site.area || '-'}</div>
                                        <div className="my-project-page-row"><span>공사 기간</span> {site.startDate} ~ {site.endDate || '미정'}</div>
                                        
                                        <div className="my-project-page-divider-line"></div>
                                        {site.contractSupply !== undefined ? (
                                            <>
                                                <div className="my-project-page-row"><span>공급가액</span> {site.contractSupply.toLocaleString()} 원</div>
                                                <div className="my-project-page-row"><span>부가세</span> {(site.contractVat || 0).toLocaleString()} 원</div>
                                                <div className="my-project-page-row total"><span>총 공사비</span> {(site.budget || 0).toLocaleString()} 원</div>
                                                
                                                {changeAmt !== 0 && (
                                                    <div className="my-project-page-row" style={{marginTop:'5px'}}>
                                                        <span>추가/변경 공사비</span>
                                                        <span style={{
...
                                    <div className="my-project-page-section">
                                        <h4>🏢 담당 파트너</h4>
                                        <div className="my-project-page-partner-box">
                                            <div className="my-project-page-p-name">{site.partnerName}</div>
                                            {/* [수정] 연락처 제거됨 */}
                                            <button className="my-project-page-btn-chat-partner" onClick={() => setIsChatOpen(true)}>
...
                                <div className="my-project-page-quick-links">
                                    <button className="my-project-page-link-btn" onClick={() => handleButtonClick('schedule', site)}>📅 공사 일정</button>
                                    <button className="my-project-page-link-btn" onClick={() => handleButtonClick('worklog', site)}>📝 작업 일지</button>
                                    <button className="my-project-page-link-btn" onClick={() => handleButtonClick('files', site)}>📂 공사자료 열람</button>
                                    <button className="my-project-page-link-btn" onClick={() => handleButtonClick('changeOrder', site)}>💰 추가/변경 견적</button>
                                    <button className="my-project-page-link-btn" onClick={() => handleButtonClick('estimate', site)}>📑 견적서 확인</button>
                                    <button className="my-project-page-link-btn" onClick={() => handleButtonClick('contract', site)}>📜 전자계약서</button>
                                    <button className="my-project-page-link-btn" onClick={() => handleButtonClick('insurance', site)}>🛡️ 보험가입 요청</button>
                                </div>
                            </div>
                        </div>
                        );
