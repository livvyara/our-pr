...
const AccountingCashReceiptPage: React.FC = () => {
  ...
  const [paymentModalTarget, setPaymentModalTarget] = useState<CashReceipt | null>(null);
  ...
  const fetchSites = async (uid: string) => { /*...*/ 
      try {
        ...
        const list: Site[] = [];
        snap.forEach(doc => list.push({ id: doc.id, name: doc.data().siteName, status: doc.data().status || '공사중' }));
        setSiteList(list);
      } catch (e) {}
  };
  ...
  const calculateSummaryManually = async () => {
      ...
      const createQuery = (colName: string) => query(collection(db, 'users', currentUid, colName), where('tradeDate', '>=', searchStartDate), where('tradeDate', '<=', searchEndDate));
      ...
      const sSnap = await getDocs(createQuery('CASH_SALES')); calc(sSnap, true);
      const pSnap = await getDocs(createQuery('CASH_PURCHASE')); calc(pSnap, false);
  ...
  const fetchData = async (isReset: boolean) => {
    ...
    try {
        ...
        const createListQuery = (collectionName: string, lastDoc: DocumentSnapshot | null) => {
            ...
            let q = query(colRef, where('tradeDate', '>=', searchStartDate), where('tradeDate', '<=', searchEndDate), orderBy('tradeDate', 'desc'), limit(LIMIT_PER_PAGE * 2));
        ...
        const processSnapshot = (snap: any, inOut: '매출' | '매입') => {
            ...
            snap.forEach((doc: any) => {
                ...
                items.push({
                    id: doc.id, tradeDate: d.tradeDate, type: d.type, inOut: inOut, franchiseName: d.franchiseName, franchiseRegNo: d.franchiseRegNo || '',
                    approvalNo: d.approvalNo, supplyAmount: Number(d.supplyAmount) || 0, taxAmount: Number(d.taxAmount) || 0,
                    serviceAmount: Number(d.serviceAmount) || 0, totalAmount: Number(d.totalAmount) || 0, remark: d.remark || '',
                    siteId: d.siteId || '', category1: d.category1 || '', category2: d.category2 || '', remark2: d.remark2 || '', linkedTransactionId: d.linkedTransactionId || ''
                } as CashReceipt);
            });
            return items;
        };
        const sSnap = await getDocs(createListQuery('CASH_SALES', currentLastSales)); newItems.push(...processSnapshot(sSnap, '매출'));
        const pSnap = await getDocs(createListQuery('CASH_PURCHASE', currentLastPurchase)); newItems.push(...processSnapshot(pSnap, '매입'));
  ...
  const handleLinkTransaction = async (id: string, inOut: '매출'|'매입', transactionId: string) => {
    if (!currentUid) return;
    setList(prev => prev.map(item => item.id === id ? { ...item, linkedTransactionId: transactionId } : item));
    try {
        const collectionName = inOut === '매출' ? 'CASH_SALES' : 'CASH_PURCHASE';
        const docRef = doc(db, 'users', currentUid, collectionName, id);
  ...
  return (
    <div className="hometax-page-container">
      <div className="hometax-header-wrapper">
          <div className="hometax-title"><h2>현금영수증 조회</h2><p>홈택스 현금영수증(매출/매입) 내역을 조회하고 관리합니다. (기본: 최근 30일)</p></div>
          <div className="hometax-control-panel">
              ...
              <div className="filter-row">
                  ...
                  <div className="filter-item"><input type="text" placeholder="가맹점명 검색" value={searchVendor} onChange={e=>setSearchVendor(e.target.value)} style={{width: '150px'}} /></div>
      ...
      <div className="summary-section">
          <div className="summary-card sales"><div className="card-header">🔵 현금 매출 ({summary.salesCount}건)</div><div className="card-body"><div className="row"><span>공급가액</span> <strong>{summary.salesSupply.toLocaleString()}</strong></div><div className="row"><span>세액</span> <strong>{summary.salesTax.toLocaleString()}</strong></div><div className="row total"><span>합계금액</span> <strong>{summary.salesTotal.toLocaleString()}</strong></div></div></div>
          <div className="summary-card purchase"><div className="card-header">🔴 현금 매입 ({summary.purchaseCount}건)</div><div className="card-body"><div className="row"><span>공급가액</span> <strong>{summary.purchaseSupply.toLocaleString()}</strong></div><div className="row"><span>세액</span> <strong>{summary.purchaseTax.toLocaleString()}</strong></div><div className="row total"><span>합계금액</span> <strong>{summary.purchaseTotal.toLocaleString()}</strong></div></div></div>
      </div>
      ...
      <div className="hometax-result-section">
        <div className="result-table-wrapper">
          <table className="hometax-table">
            <thead>
              <tr>
                <th style={{width:'160px'}}>거래일시</th><th style={{width:'50px'}}>구분</th><th style={{width:'80px'}}>유형</th>
                <th style={{textAlign:'center', width:'140px'}}>가맹점(거래처)</th>
                <th style={{textAlign:'center', width:'90px', ...separatorStyle, borderRight:'none'}}>공급가액</th><th style={{textAlign:'center', width:'80px', ...separatorStyle, borderLeft:'none', borderRight:'none'}}>세액</th><th style={{textAlign:'center', width:'90px', ...separatorStyle, borderLeft:'none'}}>합계금액</th>
                <th style={{width:'80px'}}>결제연결</th><th style={{width:'150px'}}>현장 귀속</th><th style={{width:'110px'}}>1차 분류</th><th style={{width:'110px'}}>2차 분류</th><th style={{width:'250px'}}>메모</th><th style={{width:'120px'}}>비고</th>
              </tr>
            </thead>
            <tbody>
              {list.length === 0 ? (
                loading ? (<tr><td colSpan={13} style={{textAlign:'center', padding:'50px'}}>데이터를 불러오는 중입니다...</td></tr>) : (<tr><td colSpan={13} className="no-data">조회된 내역이 없습니다.</td></tr>)
              ) : (
                list.map((item) => {
                  ...
                  return (
                    <tr key={item.id} style={{backgroundColor: rowBgColor}}>
                      <td style={{textAlign:'center'}}><div style={{display:'flex', alignItems:'center', justifyContent:'center'}}>{isPaid ? <span className="payment-status-badge paid">결제완료</span> : <span className="payment-status-badge unpaid">미결제</span>}<span style={{whiteSpace:'nowrap'}}>{item.tradeDate}</span></div></td>
                      ...
                      <td className="vendor-name-cell" title={item.franchiseName} onClick={() => setSelectedReceipt(item)} style={{textAlign: 'center', maxWidth: '140px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis'}}>{item.franchiseName}</td>
                      ...
                      <td style={{fontSize:'12px', color:'#999', maxWidth:'120px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}} title={item.remark}>{item.remark}</td>
                    </tr>
                  );
                })
              )}
      ...
      {selectedReceipt && <CashReceiptModal receipt={selectedReceipt} onClose={() => setSelectedReceipt(null)} onUpdate={(field, value) => handleFieldChange(selectedReceipt.id, selectedReceipt.inOut, field, value)} />}
      ...
      {paymentModalTarget && (
        /* [Fix 2: invoice -> receipt prop] */
        <PaymentConnectionModal 
            receipt={paymentModalTarget} 
            currentUserUid={currentUid || ''} 
            onClose={() => setPaymentModalTarget(null)} 
            onConfirm={(txId) => handleLinkTransaction(paymentModalTarget.id, paymentModalTarget.inOut, txId)} 
        />
      )}
    </div>
  );
...
// [Fix 2: PaymentConnectionModal accepts receipt prop now]
const PaymentConnectionModal: React.FC<{ 
    receipt: CashReceipt, 
    currentUserUid: string, 
    onClose: () => void, 
    onConfirm: (transactionId: string) => void 
}> = ({ receipt, currentUserUid, onClose, onConfirm }) => {
    const [transactions, setTransactions] = useState<BankTransaction[]>([]);
    const [searchDateStart, setSearchDateStart] = useState(receipt.tradeDate); 
    const [searchDateEnd, setSearchDateEnd] = useState(receipt.tradeDate);
    const [searchKeyword, setSearchKeyword] = useState(''); 
    const [searchAmount, setSearchAmount] = useState<number | string>(receipt.totalAmount); 
        ...
        <div className="invoice-modal-backdrop" onClick={onClose} style={{zIndex: 3000}}>
            <div className="invoice-paper" onClick={e => e.stopPropagation()} style={{width: '700px', height:'600px', display:'flex', flexDirection:'column'}}>
                <div style={{borderBottom:'1px solid #eee', paddingBottom:'15px', marginBottom:'15px'}}>
                    <h3 style={{margin:0}}>결제 내역 연결</h3>
                    <p style={{margin:'5px 0 0 0', fontSize:'13px', color:'#666'}}>
                        현금영수증: <strong>{receipt.tradeDate} / {receipt.franchiseName} / {receipt.totalAmount.toLocaleString()}원</strong>
                    </p>
                </div>
                <div style={{background:'#f8f9fa', padding:'15px', borderRadius:'5px', marginBottom:'15px'}}>
                    <div style={{display:'flex', gap:'10px', marginBottom:'10px'}}>
                        <input type="date" value={searchDateStart} onChange={e=>setSearchDateStart(e.target.value)} style={{padding:'5px', border:'1px solid #ddd', borderRadius:'4px'}} />
                        <span>~</span>
                        <input type="date" value={searchDateEnd} onChange={e=>setSearchDateEnd(e.target.value)} style={{padding:'5px', border:'1px solid #ddd', borderRadius:'4px'}} />
                        <button onClick={fetchTransactions} style={{padding:'5px 15px', background:'#333', color:'#fff', border:'none', borderRadius:'4px', cursor:'pointer'}}>조회</button>
                    </div>
                    <div style={{display:'flex', gap:'10px'}}>
                        <input type="text" placeholder="예금주/적요" value={searchKeyword} onChange={e=>setSearchKeyword(e.target.value)} style={{padding:'5px', border:'1px solid #ddd', borderRadius:'4px', flex:1}} />
                        <input type="number" placeholder="금액" value={searchAmount} onChange={e=>setSearchAmount(e.target.value)} style={{padding:'5px', border:'1px solid #ddd', borderRadius:'4px', width:'120px'}} />
                    </div>
                </div>
                <div style={{flex:1, overflowY:'auto', border:'1px solid #eee', borderRadius:'5px'}}>
                    <table className="hometax-table" style={{border:'none'}}>
                        <thead style={{position:'sticky', top:0, background:'#f1f1f1'}}>
                            <tr><th style={{width:'40px'}}>선택</th><th>거래일자</th><th>은행</th><th>적요/예금주</th><th>입/출금</th><th>금액</th></tr>
                        </thead>
                        <tbody>
                            {isLoading ? <tr><td colSpan={6} style={{textAlign:'center', padding:'30px'}}>조회 중...</td></tr> :
                             transactions.length === 0 ? <tr><td colSpan={6} style={{textAlign:'center', padding:'30px'}}>조회된 거래내역이 없습니다.</td></tr> :
                             transactions.map(tx => (
                                <tr key={tx.id} onClick={() => setSelectedTxId(tx.id)} style={{cursor:'pointer', backgroundColor: selectedTxId === tx.id ? '#e3f2fd' : 'transparent'}}>
                                    <td style={{textAlign:'center'}}><input type="radio" checked={selectedTxId === tx.id} onChange={() => setSelectedTxId(tx.id)} /></td>
                                    <td style={{textAlign:'center'}}>{tx.date}</td><td style={{textAlign:'center'}}>{tx.bankName}</td><td style={{textAlign:'center'}}>{tx.content}</td>
                                    <td style={{textAlign:'center', color: tx.inOut === '입금' ? 'blue' : 'red'}}>{tx.inOut}</td>
                ...
                <div style={{marginTop:'20px', textAlign:'right', display:'flex', justifyContent:'flex-end', gap:'10px'}}>
                    ...
                    <button onClick={() => selectedTxId && onConfirm(selectedTxId)} className="btn-save" style={{background: K_BRAND_COLOR, opacity: selectedTxId ? 1 : 0.5, cursor: selectedTxId ? 'pointer' : 'not-allowed'}} disabled={!selectedTxId}>연결 확인</button>
                </div>
            </div>
        </div>
    );
...
// [Fix 1: Removed issueType from Modal]
        ...
        <div className="invoice-modal-backdrop" onClick={onClose}>
            <div className={`invoice-paper ${colorClass}`} onClick={e => e.stopPropagation()} style={{width:'600px'}}> 
                <div className="invoice-header">
                    ...
                    <div className="approval-no">승인번호: {receipt.approvalNo} <br/><span style={{fontSize:'11px', color:'#888'}}>({receipt.type})</span></div>
                </div>
                <div className="invoice-body">
                    ...
                    <div className="invoice-footer-section">
                        <div className="remarks-row">
                            <div className="remark-box"><label>비고 (홈택스)</label><div className="text-content">{receipt.remark || "-"}</div></div>
                            <div className="remark-box user-memo"><label>메모 (비고2)</label><textarea className="memo-input" placeholder="사용자 메모 입력..." value={memo} onChange={(e) => setMemo(e.target.value)} onBlur={() => { if (memo !== receipt.remark2) onUpdate('remark2', memo); }} /></div>
                        </div>
                    </div>
                </div>
