...
export interface CashReceipt {
  id: string; 
  tradeDate: string; // 날짜 필드명 tradeDate
  writeDate: string; // 호환성 위해 추가
  type: string; 
  inOut: '매출' | '매입'; 
...
const AccountingCashReceiptPage: React.FC = () => {
      ...
      if (user) {
        try {
            ...
            if(userDoc.exists()) {
                ...
                // 초기 로딩
                fetchData(true, targetUid);
                calculateSummaryManually(targetUid);
            }
  ...
  useEffect(() => { 
      if (currentUid) { 
          fetchData(true, currentUid); 
          calculateSummaryManually(currentUid); 
      } 
  }, [searchStartDate, searchEndDate, searchType, showUnassigned, showAssigned, searchSiteId, searchKeyword]);
  ...
  const addDetailLog = async (item: CashReceipt, actionSuffix: string) => {
      if (!currentUid) return;
      try {
          const dateText = item.tradeDate; // YYYY-MM-DD
          const message = `[현금영수증] ${currentUserInfo.name}님이 ${dateText} ${item.franchiseName}의 ${item.inOut} 건${actionSuffix}`;
          await addDoc(collection(db, 'users', currentUid, 'ACTIVITY_LOGS'), {
              text: message, createdAt: serverTimestamp(), type: 'cash_receipt_update'
          });
      } catch (e) {}
  };
  ...
  // [수정] 두 컬렉션(CASH_SALES, CASH_PURCHASE) 합계 계산
  const calculateSummaryManually = async (uid: string) => {
      ...
      const calc = (snap: any, isSales: boolean) => {
          snap.forEach((d: any) => {
              ...
              if (isSales) { sCount++; sSupply+=supply; sTax+=tax; sTotal+=total; } 
              else { pCount++; pSupply+=supply; pTax+=tax; pTotal+=total; }
          });
      };

      if (searchType === 'all' || searchType === '매출') { 
          const sSnap = await getDocs(createQuery('CASH_SALES')); 
          calc(sSnap, true); 
      }
      if (searchType === 'all' || searchType === '매입') { 
          const pSnap = await getDocs(createQuery('CASH_PURCHASE')); 
          calc(pSnap, false); 
      }
  ...
  // [수정] 두 컬렉션(CASH_SALES, CASH_PURCHASE) 데이터 조회 및 병합
  const fetchData = async (isReset: boolean, uid: string) => {
    setLoading(true);
    try {
        ...
        const createListQuery = (colName: string, lastDoc: DocumentSnapshot | null) => {
            const colRef = collection(db, 'users', uid, colName);
            let q = query(colRef, where('tradeDate', '>=', searchStartDate), where('tradeDate', '<=', searchEndDate), orderBy('tradeDate', 'desc'), limit(LIMIT_PER_PAGE));
        ...
        const processSnapshot = (snap: any, inOut: '매출' | '매입') => {
            ...
            snap.forEach((doc: any) => {
                const d = doc.data();
                // 검색어 필터
                if (searchKeyword && !d.franchiseName.includes(searchKeyword)) return;
        ...
        if (searchType === 'all' || searchType === '매출') {
            const sSnap = await getDocs(createListQuery('CASH_SALES', currentLastSales)); 
            if (!sSnap.empty) currentLastSales = sSnap.docs[sSnap.docs.length - 1];
            newItems.push(...processSnapshot(sSnap, '매출'));
        }
        if (searchType === 'all' || searchType === '매입') {
            const pSnap = await getDocs(createListQuery('CASH_PURCHASE', currentLastPurchase)); 
  ...
  const handleFieldChange = async (id: string, inOut: '매출'|'매입', field: string, value: string) => {
      ...
      try {
          ...
          if (['siteId', 'category1', 'category2'].includes(field)) calculateSummaryManually(currentUid);
      } catch (e) { console.error("저장 실패:", e); }
  };

  // [수정] 결제 연결 시 컬렉션 동적 처리
  ...
  return (
    <div className="hometax-page-container">
      <div className="hometax-header-wrapper">
          <div className="hometax-title"><h2>현금영수증 조회</h2><p>홈택스에서 수집한 현금영수증 내역을 조회하고 현장 및 공정을 분류합니다. (기본: 최근 30일)</p></div>
          <div className="hometax-control-panel">
              ...
              <div className="filter-row">
                  ...
                  <div className="filter-item"><select value={searchType} onChange={e=>setSearchType(e.target.value as any)}><option value="all">전체 구분</option><option value="매출">매출</option><option value="매입">지출(매입)</option></select></div>
                  ...
                  <div className="filter-item"><input type="text" placeholder="가맹점명 검색" value={searchKeyword} onChange={e=>setSearchKeyword(e.target.value)} style={{width: '150px'}} /></div>
                  <button className="btn-search" onClick={() => currentUid && fetchData(true, currentUid)}>조회</button>
              </div>
          </div>
      </div>

      <div className="summary-section">
          <div className="summary-card sales"><div className="card-header">🔵 매출 합계 ({summary.salesCount}건)</div><div className="card-body"><div className="row total"><span>합계금액</span> <strong>{summary.salesTotal.toLocaleString()}</strong></div></div></div>
          <div className="summary-card purchase"><div className="card-header">🔴 매입(지출) 합계 ({summary.purchaseCount}건)</div><div className="card-body"><div className="row total"><span>합계금액</span> <strong>{summary.purchaseTotal.toLocaleString()}</strong></div></div></div>
      </div>
      ...
      <div className="hometax-result-section">
        <div className="result-table-wrapper">
          <table className="hometax-table">
            <thead>
              <tr>
                ...
                <th style={{textAlign:'center', width:'180px'}}>가맹점(거래처)</th>
            ...
            <tbody>
              {list.length === 0 ? (
                loading ? (<tr><td colSpan={11} style={{textAlign:'center', padding:'50px'}}>데이터를 불러오는 중입니다...</td></tr>) : (<tr><td colSpan={11} className="no-data">조회된 내역이 없습니다.</td></tr>)
              ) : (
                list.map((item) => {
                  ...
                  return (
                    <tr key={item.id} style={{backgroundColor: rowBgColor}}>
                      ...
                      <td className="vendor-name-cell" title={item.franchiseName} onClick={() => setSelectedReceipt(item)} style={{textAlign: 'center', maxWidth: '180px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis'}}>{item.franchiseName}</td>
          ...
          {hasMore && !loading && list.length > 0 && <div style={{display:'flex', justifyContent:'center', padding:'20px'}}><button onClick={() => fetchData(false, currentUid!)} style={{padding:'10px 40px', backgroundColor: K_BRAND_COLOR || '#1976d2', color:'#fff', border:'none', borderRadius:'5px', cursor:'pointer', fontWeight:'bold', fontSize:'14px'}}>더보기 ({LIMIT_PER_PAGE}개 로드)</button></div>}
      ...
      {paymentModalTarget && <PaymentConnectionModal item={paymentModalTarget} currentUserUid={currentUid || ''} onClose={() => setPaymentModalTarget(null)} onConfirm={(txId) => handleLinkTransaction(paymentModalTarget.id, paymentModalTarget.inOut, txId)} />}
    </div>
  );
};

// Sub Components (기존 코드 유지)
...
const PaymentConnectionModal: React.FC<{ item: CashReceipt, currentUserUid: string, onClose: () => void, onConfirm: (transactionId: string) => void }> = ({ item, currentUserUid, onClose, onConfirm }) => {
    const [transactions, setTransactions] = useState<BankTransaction[]>([]);
    const [searchDateStart, setSearchDateStart] = useState(item.tradeDate);
    const [searchDateEnd, setSearchDateEnd] = useState(item.tradeDate);
    const [searchKeyword, setSearchKeyword] = useState('');
    const [searchAmount, setSearchAmount] = useState<number | string>(item.totalAmount);
    ...
    return (
        <div className="invoice-modal-backdrop" onClick={onClose} style={{zIndex: 3000}}>
            <div className="invoice-paper" onClick={e => e.stopPropagation()} style={{width: '700px', height:'600px', display:'flex', flexDirection:'column'}}>
                <div style={{borderBottom:'1px solid #eee', paddingBottom:'15px', marginBottom:'15px'}}><h3 style={{margin:0}}>결제 내역 연결</h3><p style={{margin:'5px 0 0 0', fontSize:'13px', color:'#666'}}>대상: <strong>{item.tradeDate} / {item.franchiseName} / {item.totalAmount.toLocaleString()}원</strong></p></div>
